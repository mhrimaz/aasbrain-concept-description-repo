...
class AssetInformation(BaseModel):
    ...
    def to_rdf(self, graph=None, parent_uri="",blank_node=False):
        ...
        graph.add((node, rdflib.URIRef(AASNameSpace.AAS['AssetInformation/globalAssetId']), rdflib.Literal(self.globalAssetId)))
        graph.add(
            (
                node,
    ...
    def from_rdf(graph:Graph,subject):
        payload = {}
        asset_kind_uriref:rdflib.URIRef = next(graph.objects(subject=subject,predicate=AASNameSpace.AAS["AssetInformation/assetKind"]),None)
        if asset_kind_uriref:
            payload['assetKind'] = asset_kind_uriref[asset_kind_uriref.rfind('/')+1:]
        asset_type_uriref: rdflib.Literal = next(
            graph.objects(
                subject=subject, predicate=AASNameSpace.AAS["AssetInformation/assetType"]
            ),
            None,
        )
        ...
        global_asset_id_uriref: rdflib.Literal = next(
            graph.objects(
                subject=subject, predicate=AASNameSpace.AAS["AssetInformation/globalAssetId"]
            ),
            None,
        )
